...
(** Module [NativeInfo] allows to manipulate abstract data structures
...
(** [parse_jvmti_callstrace_file callsfile] parses the file [callsfile]
   previously created by the CallsTracer agent and returns a representation
   used by module [NativeInfo]. *)
val parse_jvmti_callstrace_file : string -> NativeInfo.t
  
(** [parse_native_info_file infofile] builds a type [NativeInfo.t] from the file
   [infofile]. Files like [infofile] can be created by the function
   [fprint_native_info] of module [NativeInfo]. *)
val parse_native_info_file : string -> NativeInfo.t
