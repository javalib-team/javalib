...
let _ =
  ...
  let argspec =
    [
      ("--silent", Arg.Clear verbose, "set silent mode");
      ("-out", Arg.Set_string outdir, "set output directory");
    ]
  in
  ...
  let filenames =
    ...
    | None ->
        prerr_endline
          "Error: a list of paths to jar files separated by ':' must be \
           specified";
  ...
  let parsed = ref [] in
  let to_parse =
    [ "rt.jar"; "resources.jar"; "jce.jar"; "jsse.jar"; "charsets.jar" ]
  in
  let iter_on_jar jarfile =
    if Sys.file_exists jarfile && List.mem (Filename.basename jarfile) to_parse
    then
      iter
        (fun ioc ->
          ...
          incr count_classes;
          if !verbose then
            Printf.printf "parsing class %s in %s...\n" (cn_name cn)
