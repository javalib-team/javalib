...
let h2l_normalclass consts c c' =
  {c' with 
    j_super = c.nc_super_class;
    j_flags = AccSynchronized::(if c.nc_final then AccFinal::c'.j_flags else c'.j_flags);
    j_methods = MethodMap.fold (fun fs f l -> h2l_nmethod consts fs f::l) c.nc_methods [];
    j_fields = FieldMap.fold (fun fs f l -> h2l_cfield consts fs f::l) c.nc_fields [];
  }
