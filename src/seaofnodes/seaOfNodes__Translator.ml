...
module TranslatorState = struct
  ...
  let initial jopcodes =
    ...
    let add_node entry_point predecessors (reg_map, son) =
      ...
      (IMap.add entry_point {predecessors; last_stack= None; region; entry_point} reg_map, son')
    in
    ...
    let son =
      Cfg.fold
        (fun k preds son ->
          ...
          List.fold_left
            (fun son pred ->
              match pred with
              | Cfg.Implicit i ->
                  let {region= ri} = snd @@ IMap.find_last (fun k -> k <= i) reg_map in
        ...
        cfg son
    in
    {stack= []; reg_map; jopcodes; pc= 0; son; work_set= ISet.empty}
  ...
  let incr_pc () = Monad.State.modify (fun g -> {g with pc= Cfg.next_pc g.pc g.jopcodes})

  let add_to_workset pc = Monad.State.modify (fun g -> {g with work_set= ISet.add pc g.work_set})

  let jump_to_next_region () =
    let* g = Monad.State.get () in
    if ISet.is_empty g.work_set then return false
    else
      let pc = ISet.find_first (Fun.const true) g.work_set in
      let* () = Monad.State.set {g with pc; work_set= ISet.remove pc g.work_set} in
      return true
  ...
  let pop_from_region pc =
    ...
    | None ->
        assert false (* Invariant : Empty lists for backjumps *)
    | Some [] ->
        assert false (* Invariant : All the lists have the same size *)
    | Some (d :: ds) ->
