...
let rec data_to_bir son (key : Data.t Son.key) =
  let data = Son.get key son in
  match data with
  | Data.Const {value= n} ->
      Const (`Int (Int32.of_int n))
  | Data.BinOp {op= Binop.Add; operand1; operand2} ->
      Binop (Add `Int2Bool, data_to_bir son operand1, data_to_bir son operand2)
  | _ ->
      failwith "todo"

let control_to_bir son (key : Control.t Son.key) =
  let control = Son.get key son in
  match control with
  | Return {operand} ->
      [| Return (Some (data_to_bir son operand)) |]
  | _ ->
      failwith "todo"
