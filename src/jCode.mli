...
(** [gen_stack_map_info e cn ms is_static code] returns the same
   [code] where [c_max_stack], [c_max_locals] and [c_stack_map] have
   been updated to consistent values necessary for the Java Bytecode
   Verifier to execute properly. The environment [e] which maps each
