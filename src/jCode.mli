...
(** [gen_stack_map_info e cn ms is_static code] generates the stackmap
   frames for the [code] contained in method [ms] of class [cn],
   necessary for the Java Bytecode Verifier to execute properly. The
   generated frame list is meant to fill the field
   [code.c_stack_map]. The environment [e] which maps each class
   encountered in [code.c_code] to its superclass needs to be
